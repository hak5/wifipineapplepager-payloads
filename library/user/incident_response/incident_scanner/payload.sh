...
# Populate share/nmap by running setup.sh on your workstation before deploying.
# Falls back to the system nmap data dir if share/nmap is not present.
NMAP_DATADIR="/root/payloads/user/incident_response/incident_scanner/share/nmap"
[ ! -d "${NMAP_DATADIR}/scripts" ] && NMAP_DATADIR=""
...
if [ -n "$NMAP_DATADIR" ]; then
    LOG "[+] NSE scripts: $NMAP_DATADIR"
else
    LOG "    [!] NSE scripts not found - using basic nmap only"
    LOG "    [!] Run setup.sh to enable richer service reporting"
fi
LOG ""
...
# Nmap wrapper - uses bundled share/nmap datadir when available for NSE script support
nmap_run() {
    if [ -n "$NMAP_DATADIR" ] && [ -d "$NMAP_DATADIR" ]; then
        nmap --datadir "$NMAP_DATADIR" "$@"
    else
        nmap "$@"
    fi
}
...
AWKEOF
        ...
        # Save parsed network data for later evil twin OUI analysis
        cp /tmp/parsed_networks.txt "${REPORT_DIR}/wireless/parsed_networks.txt" 2>/dev/null
...

{
    ...
    # OS Detection using nmap TCP/IP fingerprinting (more accurate than TTL guessing)
    echo "--- OS DETECTION (nmap fingerprinting) ---"
    if check_tool nmap; then
        _os_network=$(ip route | grep -m1 "scope link" | awk '{print $1}')
        if [ -n "$_os_network" ]; then
            echo "  Scanning $_os_network ..."
            echo ""
            nmap_run -O --osscan-guess --host-timeout 10s -T4 \
                "$_os_network" 2>/dev/null | \
                grep -E "^Nmap scan|^Host is|OS details:|Running:|OS CPE:|Aggressive OS guess" | \
                sed 's/^/  /'
        else
            echo "  [!] Could not determine local network"
        fi
    else
        # Fallback to TTL-based detection if nmap not available
        _os_network=$(ip route | grep -m1 "scope link" | awk '{print $1}')
        if [ -n "$_os_network" ] && command -v fping >/dev/null 2>&1; then
            fping -c 1 -t 100 -g "$_os_network" 2>/dev/null | grep "alive" | while read line; do
            ...
            echo "  [!] nmap not available, skipping OS detection"
        fi
    fi
...
if [ "$ENABLE_SERVICE_DISCOVERY" = true ]; then
    ...
    if [ -n "$DEFAULT_IFACE" ]; then
        # Get the network address (not host IP) from ip route for clean CIDR display
        SCAN_NETWORK=$(ip route | grep "scope link" | grep "dev $DEFAULT_IFACE" | awk '{print $1}' | head -1)
    ...
    # Added timeouts to prevent hanging on embedded systems
    {
        ...
        if [ -n "$SCAN_NETWORK" ]; then
            ...
            echo ""
            timeout 30 nmap_run -sU -p 5353 --open --host-timeout 5s \
                --script dns-service-discovery \
                "$SCAN_NETWORK" 2>/dev/null | grep -E "^Nmap|^Host|5353/udp|\|" || echo "Scan completed or timed out"
            echo ""
    ...
    # Added timeouts to prevent hanging on embedded systems
    {
        ...
        if [ -n "$SCAN_NETWORK" ]; then
            ...
            timeout 30 nmap_run -sU -p 137 --open --host-timeout 5s \
                --script nbstat \
                "$SCAN_NETWORK" 2>/dev/null | grep -E "^Nmap|^Host|137/udp|\|" || echo "Scan completed or timed out"
            echo ""
            echo "TCP 139 (NetBIOS Session):"
            timeout 30 nmap_run -p 139 --open --host-timeout 5s \
                --script smb-os-discovery \
                "$SCAN_NETWORK" 2>/dev/null | grep -E "^Nmap|^Host|139/tcp|\|" || echo "Scan completed or timed out"
        else
            echo "[!] Could not determine local network"
        fi
    ...
    # Added timeouts to prevent hanging on embedded systems
    {
        ...
        if [ -n "$SCAN_NETWORK" ]; then
            ...
            echo ""
            timeout 30 nmap_run -sU -p 161 --open --host-timeout 5s \
                --script snmp-info,snmp-sysdescr \
                "$SCAN_NETWORK" 2>/dev/null | grep -E "^Nmap|^Host|161/udp|\|" || echo "Scan completed or timed out"
        else
            echo "[!] Could not determine local network"
        fi
    ...
    # Added timeouts to prevent hanging on embedded systems
    {
        ...
        if [ -n "$SCAN_NETWORK" ]; then
            ...
            SMB_HOSTS=$(timeout 30 nmap_run -p 445 --open --host-timeout 5s \
                --script smb-security-mode,smb-os-discovery,smb-vuln-ms17-010 \
                "$SCAN_NETWORK" 2>/dev/null || echo "Scan timed out")
    ...
    # General Service Enumeration (HTTP, HTTPS, SSH, RDP, and other common ports)
    {
        echo "=== GENERAL SERVICE ENUMERATION ==="
        echo "Timestamp: $(date)"
        echo ""

        if [ -n "$SCAN_NETWORK" ]; then
            # Script and version intensity scales with scan type
            if [ "$SCAN_TYPE" = "DEEP" ]; then
                _svc_scripts="banner,http-title,http-server-header,ssl-cert,ssl-heartbleed,ssl-enum-ciphers,smb-security-mode"
                _svc_version="-sV --version-intensity 3"
            else
                _svc_scripts="banner,http-title,ssl-cert"
                _svc_version=""
            fi

            echo "--- Scanning $SCAN_NETWORK ---"
            echo "Ports: 22(SSH) 23(Telnet) 80(HTTP) 443(HTTPS) 3389(RDP) 8080 8443"
            echo ""
            # shellcheck disable=SC2086
            timeout 60 nmap_run -p 22,23,80,443,3389,8080,8443 --open \
                --host-timeout 8s $_svc_version \
                --script "$_svc_scripts" \
                "$SCAN_NETWORK" 2>/dev/null | grep -E "^Nmap|^Host|^[0-9]|\|" || echo "Scan completed or timed out"
        else
            echo "[!] Could not determine local network"
        fi
    } > "${REPORT_DIR}/services/general_services.txt"
    LOG "    [OK] General service enumeration complete"

    # Printer Discovery (Raw Print 9100, IPP 631, LPD 515)
    {
        echo "=== PRINTER DISCOVERY ==="
        echo "Timestamp: $(date)"
        echo ""
        echo "Purpose: Detect network printers with exposed print ports"
        echo "Risk: Unauthenticated printers can be used to:"
        echo "  - Print arbitrary content (social engineering, paper waste)"
        echo "  - Extract device info (model, firmware, network config)"
        echo "  - Serve as a pivot point for further exploitation"
        echo ""
        ...
        if [ -n "$SCAN_NETWORK" ]; then
            ...
            # Port 9100 - Raw/JetDirect (unauthenticated, highest risk)
            echo "** RAW PRINT PORT 9100 (JetDirect) - Highest Risk **"
            PRINTER_HOSTS=$(timeout 30 nmap_run -p 9100 --open \
                --host-timeout 5s \
                --script banner \
                "$SCAN_NETWORK" 2>/dev/null)
            echo "$PRINTER_HOSTS" | grep -E "^Nmap scan|^Host is|9100/tcp|\|" | sed 's/^/  /'
            PORT9100_COUNT=$(echo "$PRINTER_HOSTS" | grep -c "9100/tcp open" 2>/dev/null); PORT9100_COUNT=${PORT9100_COUNT:-0}
            echo "  Printers found: $PORT9100_COUNT"
            echo ""

            # Port 631 - IPP (Internet Printing Protocol)
            echo "** IPP PORT 631 (Internet Printing Protocol) **"
            timeout 30 nmap_run -p 631 --open \
                --host-timeout 5s \
                --script http-title \
                "$SCAN_NETWORK" 2>/dev/null | grep -E "^Nmap scan|^Host is|631/tcp|\|" | sed 's/^/  /'
            echo ""

            # Port 515 - LPD/LPR (Line Printer Daemon)
            echo "** LPD/LPR PORT 515 (Line Printer Daemon) **"
            timeout 30 nmap_run -p 515 --open \
                --host-timeout 5s \
                "$SCAN_NETWORK" 2>/dev/null | grep -E "^Nmap scan|^Host is|515/tcp" | sed 's/^/  /'
            echo ""

            echo "==================================="
            echo "PRINTER DISCOVERY SUMMARY"
            echo "==================================="
            echo "  Raw Print (9100): $PORT9100_COUNT printer(s) found"
            if [ "$PORT9100_COUNT" -gt 0 ]; then
                echo ""
                echo "  [!] SECURITY RISK: Unauthenticated raw print port exposed"
                echo "  [!] Affected hosts:"
                echo "$PRINTER_HOSTS" | grep "Nmap scan report" | awk '{print $NF}' | tr -d '()' | while read ip; do
                    echo "      - $ip:9100 (no authentication required)"
                done
                echo ""
                echo "  Proof of concept: echo 'test' | nc -w 2 <printer_ip> 9100"
            else
                echo "  [OK] No unauthenticated printers detected"
            fi
    ...
    } > "${REPORT_DIR}/services/printer_discovery.txt"
    LOG "    [OK] Printer discovery complete"

    # Add to severity scoring
    PRINTER_COUNT=$(grep -c "9100/tcp open" "${REPORT_DIR}/services/printer_discovery.txt" 2>/dev/null); PRINTER_COUNT=${PRINTER_COUNT:-0}
    if [ "$PRINTER_COUNT" -gt 0 ]; then
        LOG "    [!] ${PRINTER_COUNT} printer(s) with exposed raw print port found"
        for i in $(seq 1 $PRINTER_COUNT); do
            add_finding "MEDIUM" "Printer with unauthenticated raw print port 9100 exposed (JetDirect)"
        done
    fi
...
channel_hopper() {
    ...
    while true; do
        for channel in $all_channels; do
            ...
            echo "$channel" > /tmp/ir_scan_channel
            sleep "$interval"
        done
    done
}
...
while [ $(date +%s) -lt $capture_end ]; do
    ...
    if [ "$CHANNEL_HOP" = true ] && [ -n "$RECON_MON" ]; then
        current_channel=$(cat /tmp/ir_scan_channel 2>/dev/null)
...
# Summary
    ...
    # Write Wireshark filter reference file
    cat > "${REPORT_DIR}/pcaps/wireshark_filters.txt" << 'WSEOF'
╔══════════════════════════════════════════════════════════════════════╗
║           WIRESHARK FILTERS — INCIDENT RESPONSE & PEN TESTING        ║
╚══════════════════════════════════════════════════════════════════════╝

How to use:
  1. Open the indicated .pcap file in Wireshark
  2. Paste the filter into the Display Filter bar and press Enter
  3. For tshark: tshark -r <file>.pcap -Y "<filter>"

PCap files in this folder:
  beacons_*.pcap         — 802.11 beacon frames (AP advertisements)
  probe_requests_*.pcap  — 802.11 probe requests (clients searching for networks)
  wireless_recon_*.pcap  — Full RF capture (all 802.11 frame types)
  local_network_*.pcap   — Wired LAN traffic (br-lan)

═══════════════════════════════════════════════════════════════════════
  WIRELESS / 802.11 FILTERS
═══════════════════════════════════════════════════════════════════════

-- All 802.11 management frames
   [FILE: wireless_recon_*.pcap]
   wlan.fc.type == 0

-- Beacon frames only (AP advertisements)
   [FILE: beacons_*.pcap]
   wlan.fc.type_subtype == 0x0008

-- Probe requests (devices searching for networks)
   [FILE: probe_requests_*.pcap]
...
-- Deauthentication frames (possible attack / disconnect)
   [FILE: wireless_recon_*.pcap]
...
-- All frames from a specific client MAC (replace MAC below)
   [FILE: wireless_recon_*.pcap or probe_requests_*.pcap]
   wlan.sa == aa:bb:cc:dd:ee:ff

-- All frames to/from a specific BSSID/AP (replace MAC below)
   [FILE: beacons_*.pcap or wireless_recon_*.pcap]
   wlan.bssid == aa:bb:cc:dd:ee:ff

-- WEP-encrypted frames (trivially crackable)
   [FILE: beacons_*.pcap or wireless_recon_*.pcap]
   wlan.wep.iv

-- Deauth flood detection (many deauths to broadcast — possible DoS)
   [FILE: wireless_recon_*.pcap]
   wlan.fc.type_subtype == 0x000c && wlan.da == ff:ff:ff:ff:ff:ff

-- Hidden SSID beacons (SSID length 0)
   [FILE: beacons_*.pcap]
   wlan.ssid == ""

-- WPA handshakes (use for offline cracking with hashcat/aircrack)
   [FILE: wireless_recon_*.pcap]
   eapol
...
-- ARP traffic (host discovery, ARP spoofing)
   [FILE: local_network_*.pcap]
   arp

-- ARP requests only (active host scanning)
   [FILE: local_network_*.pcap]
   arp.opcode == 1
...
-- Duplicate ARP replies (ARP spoofing indicator)
   [FILE: local_network_*.pcap]
   arp.opcode == 2 && arp.duplicate-address-detected

-- mDNS traffic (Apple Bonjour, service discovery)
   [FILE: local_network_*.pcap]
   mdns

-- NBNS / NetBIOS name queries (Windows device discovery)
   [FILE: local_network_*.pcap]
   nbns
...
-- DHCP traffic (lease requests, rogue DHCP detection)
   [FILE: local_network_*.pcap]
   dhcp || bootp

-- DHCP offers only (multiple servers = possible rogue DHCP)
   [FILE: local_network_*.pcap]
   dhcp.option.dhcp == 2

-- DHCP discover (new devices joining the network)
   [FILE: local_network_*.pcap]
   dhcp.option.dhcp == 1
...
-- DNS queries for suspicious TLDs
   [FILE: local_network_*.pcap]
   dns.qry.name matches "\\.(ru|cn|tk|pw|xyz|top)$"
...
-- FTP login attempts (cleartext)
   [FILE: local_network_*.pcap]
   ftp.request.command == "USER" || ftp.request.command == "PASS"

-- Telnet traffic (cleartext protocol)
   [FILE: local_network_*.pcap]
   telnet
...
-- HTTP POST requests (form submissions, login attempts)
   [FILE: local_network_*.pcap]
   http.request.method == "POST"

-- HTTP traffic to non-standard ports (C2 or exfil)
   [FILE: local_network_*.pcap]
   http && tcp.port != 80 && tcp.port != 8080

-- Cleartext HTTP containing password keyword
   [FILE: local_network_*.pcap]
...
-- TCP SYN scan (many SYNs, no full handshakes)
   [FILE: local_network_*.pcap]
   tcp.flags.syn == 1 && tcp.flags.ack == 0

-- TCP RST storm (scan or disruption)
   [FILE: local_network_*.pcap]
   tcp.flags.reset == 1
...
-- Nmap OS detection probe (unusual TCP flags)
   [FILE: local_network_*.pcap]
   tcp.flags == 0x029

-- NULL scan (no flags — stealth scan)
   [FILE: local_network_*.pcap]
   tcp.flags == 0x000
...
-- SMB traffic (lateral movement, ransomware, EternalBlue)
   [FILE: local_network_*.pcap]
   smb || smb2

-- ETERNALBLUE / MS17-010 indicator
   [FILE: local_network_*.pcap]
   smb.path contains "IPC$"

-- RDP brute force (many SYNs to port 3389)
   [FILE: local_network_*.pcap]
...
-- LLMNR poisoning (attacker responding to name queries)
   [FILE: local_network_*.pcap]
   llmnr

-- ICMP redirect (potential MITM / traffic hijack)
   [FILE: local_network_*.pcap]
   icmp.type == 5

═══════════════════════════════════════════════════════════════════════
  EXFILTRATION & C2 DETECTION
═══════════════════════════════════════════════════════════════════════

-- Large DNS responses (possible DNS tunneling)
   [FILE: local_network_*.pcap]
   dns.flags.response == 1 && dns && frame.len > 512

-- Long DNS query names (DNS exfil via subdomain encoding)
   [FILE: local_network_*.pcap]
   dns.qry.name matches "^.{50,}"

-- ICMP with large payload (ICMP tunneling)
   [FILE: local_network_*.pcap]
   icmp && data.len > 100

-- TLS to non-standard ports (encrypted C2)
   [FILE: local_network_*.pcap]
...
-- TLS Client Hello (ciphers offered by client)
   [FILE: local_network_*.pcap]
   tls.handshake.type == 1

-- TLS Server Hello (negotiated cipher)
   [FILE: local_network_*.pcap]
...
# List all unique source IPs
   [FILE: local_network_*.pcap]
   tshark -r local_network_*.pcap -T fields -e ip.src | sort -u

# List all SSIDs seen in beacon frames
   [FILE: beacons_*.pcap]
   tshark -r beacons_*.pcap -T fields -e wlan.ssid | sort -u

# List probe request SSIDs and source MACs (client device tracking)
   [FILE: probe_requests_*.pcap]
   tshark -r probe_requests_*.pcap -T fields -e wlan.sa -e wlan.ssid | sort -u

# Show all DNS queries
   [FILE: local_network_*.pcap]
   tshark -r local_network_*.pcap -Y "dns.flags.response==0" -T fields -e dns.qry.name | sort -u

# Show all HTTP hosts contacted
   [FILE: local_network_*.pcap]
   tshark -r local_network_*.pcap -Y "http.request" -T fields -e http.host | sort -u

# Detect deauths and show attacker/target MACs
   [FILE: wireless_recon_*.pcap]
   tshark -r wireless_recon_*.pcap -Y "wlan.fc.type_subtype==0x000c" -T fields -e wlan.sa -e wlan.da

# Export files transferred over HTTP
   [FILE: local_network_*.pcap]
   tshark -r local_network_*.pcap --export-objects "http,./exported_files/"

# Follow a TCP stream (replace 0 with stream index)
   [FILE: local_network_*.pcap]
   tshark -r local_network_*.pcap -z follow,tcp,ascii,0

╔══════════════════════════════════════════════════════════════════════╗
║  Generated by incident_scanner v3.0 — curtthecoder                  ║
╚══════════════════════════════════════════════════════════════════════╝
WSEOF
    LOG "    [+] Wireshark filters saved: pcaps/wireshark_filters.txt"
...
if [ "$ENABLE_BLUETOOTH_SCAN" = true ]; then
...

    {
        ...
        # Reset adapter to clear any stuck LE scanning state
        ...
        if check_tool hcitool; then
            ...
            # Pipe through while read to force line-buffered writes to file.
            # Direct redirection (> file) uses full stdio buffering which causes
            # output to sit in memory and be lost when the process is killed.
            hcitool lescan --duplicates 2>/dev/null | while IFS= read -r _ble_line; do
                echo "$_ble_line"
            done > /tmp/ble_scan_hci.txt &
            LESCAN_PID=$!

            # Let it scan for 30 seconds
            sleep 30

            # Kill the pipeline - hcitool exits via SIGPIPE when the reader dies
...

{
    ...
    echo "  Active DHCP Server Probe (broadcast-dhcp-discover):"
    if check_tool nmap; then
        _dhcp_iface="${SCAN_INTERFACE_NAME:-br-lan}"
        _dhcp_probe=$(timeout 15 nmap_run --script broadcast-dhcp-discover \
            -e "$_dhcp_iface" 2>/dev/null | \
            grep -E "Server Identifier|IP Offered|Router|Subnet Mask|Domain Name Server")
        if [ -n "$_dhcp_probe" ]; then
            echo "$_dhcp_probe" | sed 's/^/    /'
            echo "$_dhcp_probe" | grep "Server Identifier" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' | while read srv; do
                if [ -n "$expected_dhcp" ] && [ "$srv" != "$expected_dhcp" ]; then
                    echo "    [!] ROGUE DHCP SERVER CONFIRMED: $srv"
                fi
            done
        else
            echo "    [INFO] No DHCP servers found via active probe"
        fi
    else
        echo "    [!] nmap not available for active DHCP probe"
    fi
    echo ""
    ...
    ip link show | grep "link/ether" | while read line; do
        ...
        if [ $((decimal & 2)) -eq 2 ]; then
            ...
            if echo "$mac" | grep -qi "13:37"; then
                ...
                echo $(($(cat "$_pine_cnt") + 1)) > "$_pine_cnt"
            else
                echo "    [!] SUSPICIOUS: $mac ($iface) - Locally administered, non-Pineapple"
                echo "        -> May indicate MAC spoofing or virtual interface"
                echo $(($(cat "$_susp_cnt") + 1)) > "$_susp_cnt"
            fi
        fi
    done

    pineapple_mac_count=$(cat "$_pine_cnt")
    suspicious_mac_count=$(cat "$_susp_cnt")
    rm -f "$_pine_cnt" "$_susp_cnt"
    ...
    # Detect evil twin APs using BSSID OUI comparison
    echo "** EVIL TWIN ACCESS POINT DETECTION **"
    echo "Purpose: Detect duplicate SSIDs where BSSIDs come from different manufacturers"
    echo "NOTE: Same SSID on different bands/channels from the SAME manufacturer = normal"
    echo "      (dual-band routers, mesh networks). Only flags DIFFERENT manufacturer OUIs."
    echo ""

    _et_count_file=$(mktemp)
    echo 0 > "$_et_count_file"

    if [ -f "${REPORT_DIR}/wireless/parsed_networks.txt" ]; then
        # parsed_networks.txt format: BSSID|SSID|signal|enc_type
        # Group by SSID, collect unique OUIs (first 3 octets of BSSID)
        # Flag only when the same SSID has BSSIDs from different manufacturers
        awk -F'|' 'NF>=2 && $2 != "" && $2 != "<hidden>" {
            ssid = $2
            bssid = $1
            # Extract OUI (first 3 octets)
            n = split(bssid, parts, ":")
            if (n >= 3) oui = tolower(parts[1]) ":" tolower(parts[2]) ":" tolower(parts[3])
            else oui = bssid

            # Track OUIs and full BSSIDs per SSID
            if (!(ssid in ouis)) {
                ouis[ssid] = oui
                bssids[ssid] = bssid
                oui_count[ssid] = 1
            } else if (index(ouis[ssid], oui) == 0) {
                ouis[ssid] = ouis[ssid] "," oui
                bssids[ssid] = bssids[ssid] "," bssid
                oui_count[ssid]++
            }
        }
        END {
            for (ssid in oui_count) {
                if (oui_count[ssid] > 1) {
                    print ssid "|" bssids[ssid] "|" ouis[ssid]
                }
            }
        }' "${REPORT_DIR}/wireless/parsed_networks.txt" | sort | \
        while IFS='|' read ssid bssid_list oui_list; do
            echo "  [!!!] POTENTIAL EVIL TWIN DETECTED"
            echo "        SSID: '$ssid'"
            echo "        BSSIDs with different manufacturers:"
            echo "$bssid_list" | tr ',' '\n' | while read b; do
                echo "          - $b"
            done
            echo "        OUIs (manufacturers): $oui_list"
            echo "        -> Same SSID broadcast by different hardware vendors"
            echo ""
            echo $(($(cat "$_et_count_file") + 1)) > "$_et_count_file"
        done

        _et_total=$(cat "$_et_count_file")
        if [ "${_et_total:-0}" -eq 0 ]; then
            echo "  [OK] No evil twin APs detected"
            echo "  [INFO] Duplicate SSIDs found are from the same manufacturer"
            echo "         (consistent with dual-band routers or mesh networks)"
        fi
    else
        echo "  [INFO] WiFi scan data not available (QUICK scan mode)"
    fi
...
# ============================================================================
# VULNERABILITY SCAN (DEEP scan only)
# ============================================================================

if [ "$SCAN_TYPE" = "DEEP" ]; then
    LOG ""
    LOG "[*] Running vulnerability scan on live hosts..."

    {
        echo "=== VULNERABILITY SCAN ==="
        echo "Timestamp: $(date)"
        echo ""
        echo "Scope: Live hosts on local network"
        echo "Scripts: nmap vuln category"
        echo ""

        _vuln_iface=$(ip route | grep "^default" | head -1 | awk '{print $5}')
        _vuln_network=""
        if [ -n "$_vuln_iface" ]; then
            _vuln_network=$(ip addr show "$_vuln_iface" | grep "inet " | head -1 | awk '{print $2}')
        fi
        [ -z "$_vuln_network" ] && _vuln_network=$(ip route | grep "scope link" | grep -v "br-lan\|127" | head -1 | awk '{print $1}')

        if check_tool nmap && [ -n "$_vuln_network" ]; then
            echo "--- Scanning $_vuln_network ---"
            echo ""
            timeout 120 nmap_run --script vuln \
                --host-timeout 15s -T4 \
                "$_vuln_network" 2>/dev/null | \
                grep -E "^Nmap|^Host|VULNERABLE|CVE-|Risk factor:|Description:|\|" | \
                sed 's/^/  /'
        elif [ -z "$_vuln_network" ]; then
            echo "[!] Could not determine network to scan"
        else
            echo "[!] nmap not available"
        fi

    } > "${REPORT_DIR}/analysis/vuln_scan.txt"

    VULN_COUNT=$(grep -c "VULNERABLE" "${REPORT_DIR}/analysis/vuln_scan.txt" 2>/dev/null); VULN_COUNT=${VULN_COUNT:-0}
    if [ "$VULN_COUNT" -gt 0 ]; then
        LOG "    [!!!] ${VULN_COUNT} vulnerabilities found - REVIEW IMMEDIATELY"
        for i in $(seq 1 $VULN_COUNT); do
            add_finding "CRITICAL" "Vulnerability detected by nmap vuln scan"
        done
    else
...
if [ "$ENABLE_RECON_DB_ANALYSIS" = true ]; then
    ...
    if [ -n "$RECON_DB" ] && check_tool sqlite3; then
        # Helper: run sqlite3 with lock timeout so recon module doesn't block us
        _sq() { sqlite3 -timeout 5000 "$RECON_DB" "$@" 2>/dev/null; }

        {
            ...
            # Quick diagnostic — confirm tables and raw row counts
            echo "  [DB Tables]"
            _sq "SELECT '    ' || name || ': ' || (CASE name
                     WHEN 'wifi_device' THEN (SELECT COUNT(*) FROM wifi_device)
                     WHEN 'ssid'        THEN (SELECT COUNT(*) FROM ssid)
                     WHEN 'scan'        THEN (SELECT COUNT(*) FROM scan)
                     WHEN 'handshake'   THEN (SELECT COUNT(*) FROM handshake)
                     WHEN 'hostap_basic' THEN (SELECT COUNT(*) FROM hostap_basic)
                     ELSE '?' END) || ' rows'
            ...
            TOTAL_DEVICES=$(_sq "SELECT COUNT(DISTINCT mac) FROM wifi_device;")
            TOTAL_SSIDS=$(_sq "SELECT COUNT(DISTINCT ssid) FROM ssid WHERE length(ssid) > 0;")
            TOTAL_BSSIDS=$(_sq "SELECT COUNT(DISTINCT bssid) FROM ssid;")
            HIDDEN_NETS=$(_sq "SELECT COUNT(*) FROM ssid WHERE hidden=1;")
            OPEN_NETS=$(_sq "SELECT COUNT(DISTINCT bssid) FROM ssid WHERE encryption=0;")
            TOTAL_SCANS=$(_sq "SELECT COUNT(*) FROM scan;")
            ...
            BASIC_CREDS=$(_sq "SELECT COUNT(*) FROM hostap_basic;")
            if [ "${BASIC_CREDS:-0}" -gt 0 ]; then
                echo "  [!!!] CLEARTEXT CREDENTIALS CAPTURED: $BASIC_CREDS"
                echo ""
                _sq -header "SELECT type, identity, password, datetime(time, 'unixepoch') as captured_time FROM hostap_basic ORDER BY time DESC LIMIT 20;" | sed 's/^/  /'
                echo ""
            ...
            CHALRESP_COUNT=$(_sq "SELECT COUNT(*) FROM hostap_chalresp;")
            if [ "${CHALRESP_COUNT:-0}" -gt 0 ]; then
                echo "  [!!] NTLM/CHALLENGE-RESPONSE HASHES CAPTURED: $CHALRESP_COUNT"
                echo ""
                _sq -header "SELECT type, username, datetime(time, 'unixepoch') as captured_time FROM hostap_chalresp ORDER BY time DESC LIMIT 20;" | sed 's/^/  /'
                echo ""
            ...
            # WPA Handshakes
            HANDSHAKE_COUNT=$(_sq "SELECT COUNT(*) FROM handshake;")
            HOSTAP_HS_COUNT=$(_sq "SELECT COUNT(*) FROM hostap_handshake;")
            ...
            VICTIM_COUNT=$(_sq "SELECT COUNT(DISTINCT mac) FROM hostap_client;")
            if [ "${VICTIM_COUNT:-0}" -gt 0 ]; then
                ...
                echo "  ----------------|--------------------------|------------------"
                _sq "SELECT mac, ssid, datetime(connected_time, 'unixepoch') FROM hostap_client ORDER BY connected_time DESC LIMIT 25;" | while IFS='|' read mac ssid ctime; do
                    printf "  %-16s| %-24s | %s\n" "$mac" "$ssid" "$ctime"
                done
                echo ""
            ...
            if [ "${OPEN_NETS:-0}" -gt 0 ]; then
                ...
                echo "  -------------------------|--------------|-------"
                _sq "SELECT DISTINCT ssid, bssid, MAX(signal) as sig FROM ssid WHERE encryption=0 AND length(ssid) > 0 GROUP BY ssid ORDER BY sig DESC LIMIT 20;" | while IFS='|' read ssid bssid signal; do
                    printf "  %-25s| %s | %s dBm\n" "$ssid" "$bssid" "$signal"
                done
                echo ""
            else
            ...
            if [ "${HIDDEN_NETS:-0}" -gt 0 ]; then
                ...
                echo "  -------------|---------|--------|------------"
                _sq "SELECT bssid, channel, signal, encryption FROM ssid WHERE hidden=1 ORDER BY signal DESC LIMIT 20;" | while IFS='|' read bssid channel signal enc; do
                    enc_str="Unknown"
                    [ "$enc" = "0" ] && enc_str="Open"
                    [ "$enc" = "2" ] && enc_str="WEP"
            ...
            echo "  ---------------|----------|--------|----------"
            _sq "SELECT mac, SUM(packets) as total_packets, MAX(signal) as sig, freq FROM wifi_device GROUP BY mac ORDER BY total_packets DESC LIMIT 20;" | while IFS='|' read mac packets signal freq; do
            ...
            echo ""
            _sq "SELECT ssid, COUNT(DISTINCT wifi_device) as device_count FROM ssid WHERE type=1 AND length(ssid) > 0 GROUP BY ssid ORDER BY device_count DESC LIMIT 20;" | while IFS='|' read ssid count; do
            ...
            echo "═══════════════════════════════════════════════════════════════"
            echo ""
            _sq "SELECT id, name, datetime(time, 'unixepoch') as scan_time FROM scan ORDER BY time DESC LIMIT 10;" | while IFS='|' read id name stime; do
...

{
    ...
    if [ -f "${REPORT_DIR}/analysis/client_fingerprinting.txt" ]; then
        ...
        echo "  - OS Detection (nmap fingerprinting): Complete"
    ...
    if [ -f "${REPORT_DIR}/analysis/recon_intelligence.txt" ]; then
        ...
        echo "  Cumulative Historical Data (all-time from recon.db):"
        echo "    - Unique Devices Ever Seen (all scans, all time): ${RECON_DEVICES:-0}"
        echo "    - Unique SSIDs Ever Seen   (all scans, all time): ${RECON_SSIDS:-0}"
        echo ""
    ...
    echo ""
    echo "═══════════════════════════════════════════════════════════════════════"
    echo "  PACKET CAPTURE ANALYSIS GUIDE"
    echo "═══════════════════════════════════════════════════════════════════════"
    echo ""
    echo "  A ready-to-use Wireshark filter reference has been saved to:"
    echo "    pcaps/wireshark_filters.txt"
    echo ""
    echo "  It contains filters for every scenario below — each one labeled"
    echo "  with the exact .pcap file to open."
    echo ""
    echo "  PCAP FILES & WHAT TO LOOK FOR:"
    echo ""
    echo "  beacons_${TIMESTAMP}.pcap"
    echo "    -> Open in: Wireshark"
    echo "    -> Look for: AP enumeration, hidden SSIDs, WEP networks"
    echo "    -> Key filters: wlan.fc.type_subtype == 0x0008"
    echo "                    wlan.ssid == \"\"  (hidden APs)"
    echo "                    wlan.wep.iv      (WEP frames)"
    echo ""
    echo "  probe_requests_${TIMESTAMP}.pcap"
    echo "    -> Open in: Wireshark"
    echo "    -> Look for: Client device tracking, preferred network lists"
    echo "    -> Key filters: wlan.fc.type_subtype == 0x0004"
    echo "                    wlan.sa == <MAC>  (track a specific device)"
    echo ""
    echo "  wireless_recon_${TIMESTAMP}.pcap"
    echo "    -> Open in: Wireshark"
    echo "    -> Look for: Deauth attacks, WPA handshakes, rogue APs"
    echo "    -> Key filters: eapol                          (WPA handshakes)"
    echo "                    wlan.fc.type_subtype == 0x000c (deauth frames)"
    echo "                    wlan.fc.type_subtype == 0x000c && wlan.da == ff:ff:ff:ff:ff:ff"
    echo "                                                   (deauth flood/DoS)"
    echo ""
    echo "  local_network_${TIMESTAMP}.pcap"
    echo "    -> Open in: Wireshark"
    echo "    -> Look for: Credentials, DHCP rogue servers, ARP spoofing, C2"
    echo "    -> Key filters: http.authorization contains \"Basic\"  (cleartext creds)"
    echo "                    dhcp.option.dhcp == 2               (DHCP servers)"
    echo "                    arp.opcode == 2 && arp.duplicate-address-detected"
    echo "                                                        (ARP spoofing)"
    echo "                    smb || smb2                         (lateral movement)"
    echo ""
    echo "  TIP: For quick extraction without Wireshark, use tshark:"
    echo "    tshark -r beacons_${TIMESTAMP}.pcap -T fields -e wlan.ssid | sort -u"
    echo "    tshark -r probe_requests_${TIMESTAMP}.pcap -T fields -e wlan.sa -e wlan.ssid | sort -u"
    echo "    tshark -r local_network_${TIMESTAMP}.pcap -Y 'http.request' -T fields -e http.host | sort -u"
    echo ""
    echo "  Full filter list with file references: pcaps/wireshark_filters.txt"
    echo ""
