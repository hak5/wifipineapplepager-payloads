...
if ! command -v jq >/dev/null 2>&1; then
    LOG "jq is not installed. Installing..."
    opkg update
    opkg install jq
    if ! command -v jq >/dev/null 2>&1; then
...
# Loop through each site in the JSON file
for SITE_NAME in $(jq -r 'keys[]' "$SITES_JSON"); do
  ...
  # Perform the check based on the error_type
...
if [ -n "$FOUND_SITES" ]; then
    ...
    # Write each URL to the loot file on a separate line
    echo "$FOUND_URLS" | tr ' ' '\n' > "$FOUND_LOOT"

    # Log the found URLs (each on a new line)
    echo "$FOUND_URLS" | tr ' ' '\n' | while read -r url; do
        LOG green "$url"
    done
else
