...
set_broadcast_ssid() {
    ...
    # Wait for reload to complete (up to 10 seconds)
...
mode_quick_karma() {
    ...
    # Build SSID list for PROMPT
    local ssid_list="SELECT SSID TO BROADCAST:
...
Press OK then enter number"
    ...
    # Show options in PROMPT, then pick
    PROMPT "$ssid_list"

    local pick=$(NUMBER_PICKER "Enter SSID (1-$count)" 1)
    case $? in
    ...
    while true; do
        ...
        case $choice in
            1)
                ...
                local ssid=$(TEXT_PICKER "SSID name")
                case $? in
                    $DUCKYSCRIPT_CANCELLED|$DUCKYSCRIPT_REJECTED|$DUCKYSCRIPT_ERROR)
                        continue
                        ;;
...
# Mode selection - PROMPT shows options, user acknowledges, then picks
PROMPT "ATTACK MODES:

1. Quick Karma (common SSIDs)
2. Passive (gather probes)
3. Aggressive (collect+karma)
4. Custom (manual control)
5. FENRIS Chain (post-deauth)

Press OK then enter number"

mode_choice=$(NUMBER_PICKER "Enter mode (1-5)" 1)
case $? in
    $DUCKYSCRIPT_CANCELLED|$DUCKYSCRIPT_REJECTED|$DUCKYSCRIPT_ERROR)
        LOG "Cancelled"
        exit 1
